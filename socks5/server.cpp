...
int proc_socks(int in){
    ...
    SKReq sreq;
	assert(0 == recv_n(in,(char*)&sreq,sizeof(sreq)));//客户端向代理服务器发来请求
	...
	rep.ver=5;
	rep.m=0;
	assert(0==send_n(in,(char*)&rep,sizeof(rep)));//代理服务器端作出回应
    ...
    AddrReq areq;
	assert(0==recv_n(in,(char*)&areq,sizeof(areq)));//客户端要求代理服务器做什么
	assert(areq.ver==5 && areq.cmd==1 && areq.rsv==0);
	if(areq.atype==1){  //ipv4
		//IPV4
		assert(0==recv_n(in,(char*)&out_addr.sin_addr.s_addr,
	...
	memcpy(&arep.addr,&local_addr.sin_addr.s_addr,sizeof(local_addr.sin_addr.s_addr));//远程服务器分配的ip
	memcpy(&arep.port,&local_addr.sin_port,sizeof(short));//远程服务器分配的端口
	assert(send_n(in,(char*)&arep,sizeof(arep))==0);
	TParam up={in,out,0};
	TParam down={out,in,0};
	assert(0==pthread_create(&down.bro,NULL,transfer,&up));
	assert(0==pthread_create(&down.bro,NULL,transfer,&down));
